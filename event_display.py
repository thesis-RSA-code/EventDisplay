...
def load_data(file_path, tree_name, detector_geom, experiment, events_to_display='all') : # load data from root file and project it to 2D
    ...
    if 'new_twall' in events_root.keys() :
      events_dic['add_info'].append({'label': r'$t_\mathrm{wall}$', 'unit': 'cm', 'values': events_root['new_twall'].array()[event_start:event_end]})

    if 'new_dwall' in events_root.keys() :
      events_dic['add_info'].append({'label': r'$d_\mathrm{wall}$', 'unit': 'cm', 'values': events_root['new_dwall'].array()[event_start:event_end]})

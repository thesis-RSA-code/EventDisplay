


# In cm

DETECTOR_GEOM = {
  'SK': {'height': 3620.0, 'cylinder_radius': 3368.15/2, 'PMT_radius': 25.4}, 
  'HK': {'height': 6575.1, 'cylinder_radius': 6480/2, 'PMT_radius': 25.4}, 
  'HK_realistic': {'height': 6701.41, 'cylinder_radius': 3242.96, 'PMT_radius': 50},
  #'WCTE': {'height': 338.0, 'cylinder_radius': 369.6/2, 'PMT_radius': 4.0},
  'WCTE': {'height': 271.4, 'cylinder_radius': 307.6/2, 'PMT_radius': 4.0}, # WCTE in with z as cylinder axis
  'WCTE_r': {'height': 271.4, 'cylinder_radius': 307.6/2, 'PMT_radius': 4.0}, # WCTE in WCSim, with y as cylinder axis
  'DEMO': {'height': 558.92 * 2 , 'cylinder_radius': 558.92, 'PMT_radius': 25.4}
}

# 'HK': {'height': 6575.1, 'cylinder_radius': 6480/2, 'PMT_radius': 25.4}, 
# 'HK_realistic': {'height': 6701.41, 'cylinder_radius': 3242.96, 'PMT_radius': 25.4},


NORMALIZED_VALUES = {
    'time': [550.0, 1900.0],
    'charge': [0.01, 1000.],
    'hitx': [-3242.96, 3242.96],    
    'hity': [-3242.96, 3242.96],
    'hitz': [-3296.47, 3296.47]
}




